...
std::optional<Syntax::Arg> Syntax::ArgParser::TryParseVector2(std::string arg)
{
    ...
    if (std::regex_match(arg, match, vector2_regex)) {
        ...
        return Syntax::Arg::FromVector2(Core::Vector2(x, y));
    }
...
void Dispatcher::Dispatch(std::string command_string, Core::Output& stream)
{
    ...
    if (tokens.size() == 0) {
        stream.Writeln("[ERROR]: No command entered.");
        return;
    }
